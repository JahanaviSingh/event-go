  ...
  searchUniversities: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      try {
        ...
        console.log(
          '[searchUniversities] Database search found:',
          dbUniversities.length,
          'universities',
        )

        if (dbUniversities.length > 0) {
          console.log(
            '[searchUniversities] Returning database results:',
            dbUniversities,
          )
        ...
        console.log(
          '[searchUniversities] No database results, querying Overpass API...',
        )
        ...
        console.log(
          '[searchUniversities] Using simplified Overpass API query:',
          query,
        )
        ...
        try {
          ...
          console.log(
            `[searchUniversities] Response headers:`,
          ...
          console.log(
            `[searchUniversities] Found ${data.elements.length} universities`,
          )
          ...
          const universities = data.elements
            .filter((element: any) => {
              ...
              if (!hasName || !hasLocation) {
                console.log(
                  `[searchUniversities] Filtered out university:`,
                  element,
                )
              }
          ...
          console.log(
            `[searchUniversities] Processed ${universities.length} universities:`,
            universities,
          )
          ...
          for (const uni of universities) {
            try {
              ...
              console.error(
                '[searchUniversities] Database error for university:',
                {
                  id: uni.id,
                  error: dbError,
                  university: uni,
          ...
          throw new TRPCError({
            ...
            message:
              'Failed to fetch universities. Please try again in a few moments.',
            cause: error,
          })
        }
      } catch (error) {
